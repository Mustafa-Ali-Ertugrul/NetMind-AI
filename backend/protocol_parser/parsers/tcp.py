"""TCP packet parser for tshark JSON output."""
...
def parse_tcp_packet(
    packet_data: dict[str, Any],
    pcap_id: str | UUID,
) -> ParsedPacket | None:
    """Parse TCP packet from tshark JSON output.
    ...
    Returns:
        ParsedPacket object if packet contains TCP data, None otherwise.
    """
    try:
        ...
        tcp_layer = _safe_get(layers, "tcp")
        if not tcp_layer:
            return None
        ...
        # Extract TCP ports
        src_port = _parse_int(_safe_get(tcp_layer, "tcp.srcport"))
        dst_port = _parse_int(_safe_get(tcp_layer, "tcp.dstport"))

        # Extract TCP flags
        flags = _parse_str(_safe_get(tcp_layer, "tcp.flags"))
